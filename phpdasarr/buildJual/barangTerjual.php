...
function kurangiStock($post)
{
    global $connect;
    $namaBarang1 = htmlspecialchars($post["namaBarangStock"]);
    ...
    $stockData1 = mysqli_query($connect, "SELECT * FROM stock WHERE namaBarang='$namaBarang1'");
    $resultStock = mysqli_fetch_array($stockData1);
    $stockSekarang = $resultStock["stock"];
    if ($stockSekarang - $jumlahBarangTerjual < 0) {
        echo "
        <script>
            alert('Stock yang dimasukan Terlalu Banyak ATAU stock sudah habis !!!');
            document.location.href = 'barangTerjual.php';
        </script>
    ";
    } else {
        $kurangiStock = $stockSekarang - $jumlahBarangTerjual;
        var_dump($resultStock);
        $updateStock = "UPDATE stock SET stock='$kurangiStock', tanggal='$tanggal' WHERE namaBarang='$namaBarang1'";
        ...
        $query = "INSERT INTO jual (namaBarang,jumlahBarangTerjual,jumlahHargaTerjual,tanggal) VALUES ('$namaBarang1','$jumlahBarangTerjual','$jumlahHargaTerjual','$tanggal')";
...
// cek apakah tombol sumbit sudah di tekan 
if (isset($_POST["klikTerjual"])) {
    if (kurangiStock($_POST) > 0) {
        // kalo berhasil ditambahkan data
        // dan redirect kehalaman "barangMasuk" menggunakan javascript
        echo "
            <script>
                alert('data terjual berhasil dihapus');
    ...
    <div class="container" style="width:70%; margin-top:100px; height:100vh;">
        ...
        <form method="post" action="">
            <div class="mb-3">
                ...
                <select name="namaBarangStock" class="form-select" aria-label="Default select example">
            ...
            <div class="mb-3">
                <!-- ATRIBUT NAME = jumlahBarangTerjual -->
                <label for="jumlahBarangTerjual" class="form-label">JumlahBarangTerjual</label>
                <input type="text" class="form-control" id="jumlahBarangTerjual" aria-describedby required="emailHelp" name='jumlahBarangTerjual' placeholder="0">
            </div>
            <div class="mb-3">
                <!-- ATRIBUT NAME = totalHargaBeliBarang -->
                <label for="jumlahHargaTerjual" class="form-label">Jumlah Harga Terjual: <br> Contoh = 20.000</label>
                <input type="text" class="form-control" id="jumlahHargaTerjual" aria-describedby required="emailHelp" name='jumlahHargaTerjual' placeholder="0">
            </div>
            <div class="mb-3">
            ...
            <button type="submit" class="btn btn-primary" name='klikTerjual'>Tambah data</button>
        </form>
    </div>
